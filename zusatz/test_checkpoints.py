...
print("o um den checkpoint, distanz, winkel anzuzeigen und u um den nächsten checpoiint zu togglen")
...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            spawn_point = carla.Transform(carla.Location(x=x, y=y, z=z), carla.Rotation(yaw=180))
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
            self.modify_vehicle_physics(self.player)
        # Set up the sensors.
...
class KeyboardControl(object):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    world.get_next_checkpoint_state()

                    world.world.debug.draw_point(
                        world.checkpoint_manager.checkpoints[world.checkpoint_manager.current].get_location(),
                        size=1.0,
                    ...
                    world.checkpoint_manager.toggle_next()
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        if keys[K_UP] or keys[K_w]:
            self._control.throttle = 1# max(self._control.throttle + 0.01, 1.00)
        else:
...
class HUD(object):
    ...
    def notification(self, text, seconds=0.5):
...
class LaneInvasionSensor(object):
    ...
    def _on_invasion(weak_self, event):
        ...
        if not self:
            return
        lane_types = set(x for x in event.crossed_lane_markings)
        lane_types = set(map(lambda t: t.type, lane_types))
